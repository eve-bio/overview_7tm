...
approved_no_data <- hcs |>
  filter(Target_Class == "7TM", data_readiness == "No data available") |>
  left_join(
    bundles_df,
    by = join_by(bundle_id, bundle_name, Mode, Target_Class)
  ) |>
...
approved_protocols <- pins::pin_read(get_board(), 'mfearon/protocol_status') |>
  filter(Target_Class == "7TM" & Status == "Published") |>
  select(Target_Name, Assay_Mode)

# Filter to only include approved protocols
approved_no_data <- approved_no_data |>
  inner_join(approved_protocols, join_by(Target_Name, Mode == Assay_Mode))

datatable(approved_no_data, 
